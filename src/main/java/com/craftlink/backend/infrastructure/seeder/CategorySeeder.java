package com.craftlink.backend.infrastructure.seeder;
...
import com.craftlink.backend.category.infrastructure.persistance.read.JpaCategoryQueryRepository;
...
public class CategorySeeder implements CommandLineRunner {
  ...
  public void run(String... args) {
    if (jpaCategoryQueryRepository.count() == 0) {
  ...
  private void seedCategories() {
    ...
    for (CategoryData categoryData : categoriesData) {
      ...
      CategoryEntity savedCategory = jpaCategoryQueryRepository.save(category);
  ...
  private void createServicesForCategory(CategoryEntity category, List<ServiceData> servicesData) {
    for (ServiceData serviceData : servicesData) {
      ...
      service.setStatus(LifecycleStatus.ACTIVE);
