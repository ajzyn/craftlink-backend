...
public record RefreshTokenValue(String value) {
  ...
  public RefreshTokenValue {
    if (StringUtils.isBlank(value)) {
      throw new DomainViolation("INCORRECT_REFRESH_TOKEN", "Refresh token is empty", Map.of());
    }
    try {
      Base64.getUrlDecoder().decode(value);
    } catch (IllegalArgumentException e) {
      throw new DomainViolation("INCORRECT_REFRESH_TOKEN", "Invalid refresh token format");
    }
  }
}
